...
test('prompt generation does not launch provider work when no limit snapshot exists', async () => {
  await withService(
    'marker',
    async ({ service, launched }) => {
      ...
      assert.equal(launched.count, 0);
      assert.match(failed.error ?? '', /limit gate blocked/);
      assert.match(failed.error ?? '', /unavailable/i);
    },
    {
...
test('prompt generation does not launch provider work when limit snapshot is stale', async () => {
  await withService(
    'marker',
    async ({ service, launched }) => {
      const started = await service.start({ inputText: 'rough draft', agentType: 'claude' });
      ...
      assert.equal(launched.count, 0);
      assert.match(failed.error ?? '', /limit gate blocked/);
      assert.match(failed.error ?? '', /stale/i);
    },
    {
...
test('prompt generation does not launch provider work when latest provider probe failed', async () => {
  await withService(
    'marker',
    async ({ service, launched }) => {
      const started = await service.start({ inputText: 'rough draft', agentType: 'claude' });
      ...
      assert.equal(launched.count, 0);
      assert.match(failed.error ?? '', /limit gate blocked/);
      assert.match(failed.error ?? '', /probe failed/i);
    },
    {
      limitGate: new StateLimitGate(
        () =>
