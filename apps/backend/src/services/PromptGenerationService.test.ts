...
import type { runAgentTurn } from '../agents/runner.js';
...
/** Fake headless turn runner: counts launches; 'missing' mode returns no output. */
function fakeTurnRunner(launched: { count: number }, mode = 'marker'): typeof runAgentTurn {
  return async () => {
    launched.count += 1;
    return {
      state: 'completed',
      ...
      resultText: mode === 'missing' ? '' : 'Generated by fake-cli: improved prompt text.',
      result: null,
      usage: { costUsd: 0.01 },
      eventCount: 1,
      durationMs: 5,
...
async function withService(
  ...
  const service = new PromptGenerationService({
    ...
    timing: timing(),
    turnRunner: fakeTurnRunner(launched, mode),
  });
  try {
...
test('prompt generation does not launch provider work when limit gate blocks', async () => {
  ...
  const service = new PromptGenerationService({
    ...
    runtimeRoot: () => runtimeDir,
    timing: timing(),
    turnRunner: fakeTurnRunner(launched),
  });
  try {
